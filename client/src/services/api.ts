...
const videoService = {
  ...
  // Upload a file part directly to S3 using the presigned URL - ENHANCED VERSION
  uploadFilePart: async (
    ...
    try {
      ...
      // Using axios for the upload with progress tracking
      const response = await s3AxiosInstance.put(presignedUrl, file, config);
      ...
      if (!etag) {
        ...
        // Try to get the ETag from the response or generate a placeholder
        const timestamp = new Date().getTime();
        const randomString = Math.random().toString(36).substring(2, 15);
        const fallbackETag = `"${timestamp}-${randomString}"`;

        console.warn(
          "ETag not found in response headers, using fallback:",
          fallbackETag
        );
